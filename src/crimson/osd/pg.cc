  ...
  std::vector<pg_log_entry_t>&& log_entries)
{
  ...
  auto [submitted, all_completed] = co_await backend->mutate_object(
  ...
  ceph_tid_t rep_tid)
{
  ...
  return seastar::do_with(log_entries, std::move(t), set<pg_shard_t>{},
    ...
    return seastar::do_for_each(get_acting_recovery_backfill(),
      ...
      return shard_services.send_to_osd(peer.osd,
                                        std::move(log_m),
                                        get_osdmap_epoch());
    }).then([this, waiting_on, &t, rep_tid] () mutable {
