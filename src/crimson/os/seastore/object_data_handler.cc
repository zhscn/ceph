  ...
  return trans_intr::do_for_each(
	...
	return ctx.tm.alloc_extents<ObjectDataBlock>(
	  ...
	  for (auto &extent : extents) {
	    ...
	    iter.copy_deep(extent->get_length(), extent->get_bptr());
	  }
...
ObjectDataHandler::set_alloc_hint(
  context_t ctx,
  objaddr_t expected_object_size,
  objaddr_t expected_write_size,
  uint32_t flags)
{
  ...
  return with_object_data(
    ...
    [this, ctx, expected_object_size](auto &object_data) {
      return prepare_data_reservation(
	ctx, object_data,
	p2roundup(expected_object_size, ctx.tm.get_block_size()));
    });
}
