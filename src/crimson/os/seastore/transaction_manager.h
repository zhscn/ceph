...
public:
  ...
  remap_pin_ret<N> remap_pin(
	...
	original_len, intermediate_base, intermediate_key]
	    ...
	    &count, intermediate_base, intermediate_key, dec_res=std::move(dec_res),
	    original_laddr, original_paddr, original_len](auto &remap) mutable {
            LOG_PREFIX(TransactionManager::remap_pin);
            auto remap_offset = remap.offset;
            auto remap_len = remap.len;
  ...
  clone_extent_ret clone_extent(
    ...
    SUBDEBUGT(seastore_tm, "len={}, laddr_hint={}, intermediate_key {}",
...
private:
  ...
  alloc_remapped_extent_ret alloc_remapped_extent(
	...
	set_cache_info(t, cold_ext, remap_laddr);
      ...
      ceph_assert(shadow_paddr == P_ADDR_NULL);
    }
    return lba_manager->alloc_extent(
      t,
      remap_laddr,
	...
	return lba_manager->alloc_shadow_extent(
	  ...
	  cold_ext->set_laddr(sref->get_key());
