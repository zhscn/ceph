...
public:
  ...
  static laddr_t from_byte_offset(loffset_t value) {
    ...
    // avoid conflict with global metadata and onode extents.
    if ((id & layout::LocalCloneIdSpec::MAX) == 0) {
      id++;
    }
    // move the remaining bits to local object id. id is possible
    // to be greater than max local clone id, we allow its bits overflow
    // to crush field.
  ...
  bool is_onode_extent_address() const {
    return !is_global_address() && get_local_object_id() == 0;
  }
