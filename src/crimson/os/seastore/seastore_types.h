...
public:
  ...
  pool_t get_pool() const {
    return PoolSpec::get<int8_t>(value);
  }
  ...
  shard_t get_shard() const {
    return ShardSpec::get<int8_t>(value);
  }
  ...
  crush_hash_t get_crush() const {
    return CrushSpec::get<uint32_t>(value);
  }

  void set_crush(crush_hash_t crush) {
...
struct pladdr_t {
  ...
  laddr_t build_laddr(laddr_t original_laddr) const {
    ...
    return original_laddr
      .with_local_clone_id(get_local_clone_id())
      .without_recover();
  }
