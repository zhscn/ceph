...
concept has_three_way = requires(const L &l, const R &r) { l <=> r; };
...
struct laddr_t {
  ...
  static laddr_t get_metadata_hint(
    ...
    local_snap_t snap_id)
  {
    auto ret = get_data_hint(pool, shard, crush, snap_id);
  ...
  static std::pair<laddr_t, laddr_t> clone(laddr_t laddr, local_snap_t head_id) {
    auto new_head = laddr.with_local_snap_id(head_id + 2);
    auto snap = laddr.with_local_snap_id(head_id + 1);
    return std::make_pair(new_head, snap);
  }
  ...
  void set_local_snap_id(local_snap_t id) {
    LocalSnapIdSpec::set(value, id);
  }

  laddr_t with_local_snap_id(local_snap_t id) const {
    auto ret = *this;
    ret.set_local_snap_id(id);
    return ret;
  }

  laddr_t without_local_snap_id() const {
    return with_local_snap_id(LOCAL_SNAP_ID_NULL);
  }
...
private:
  ...
  template <details::has_three_way T>
...
private:
  ...
  struct FieldSpec {
    ...
    static constexpr internal128_t MASK = ((internal128_t(1) << length) - 1) << offset;
...
struct pladdr_t {
  std::variant<local_snap_t, paddr_t> pladdr;
  ...
  explicit pladdr_t(paddr_t paddr)
    : pladdr(paddr) {}
  explicit pladdr_t(local_snap_t local_snap_id)
  ...
  laddr_t build_laddr(laddr_t original_laddr) const {
    assert(pladdr.index() == 0);
    return original_laddr.with_local_snap_id(get_local_snap_id());
  }
