  ...
  remap_extent_func_t func)
{
  ...
  return get_mappings(t, src_base, length
	...
	: src_base(src_base), dst_base(dst_base), length(length),
	  remap_extent(std::move(func)), src_pins(std::move(src_pins))
      {}
    };
	...
	auto src_front_end = state.src_pins.front()->get_key() +
			     state.src_pins.front()->get_length();
	bool src_need_pop_front = (src_front_end <= intermediate_key);
	if (src_need_pop_front) {
	  state.src_pins.pop_front();
	}
	auto &s_pin = state.src_pins.front();
	...
	return load_child_ext(c.trans, iter
	  ...
	  return state.remap_extent(
	    ext.get(), iter.get_pin(c), std::move(remaps)
  ...
  remap_extent_func_t func)
{
  ...
  return with_btree_state<LBABtree, state_t>(
    ...
    [c, data_only, FNAME, this](LBABtree &btree, state_t &state) {
      return LBABtree::iterate_repeat(
        c,
	btree.upper_bound_right(c, state.src_base),
	[c, data_only, &btree, &state, FNAME, this](LBABtree::iterator &iter) {
	  if (iter.is_end() ||
	      iter.get_key() >= state.get_src_end()) {
	  ...
	  return load_child_ext(c.trans, iter
		...
		if (split_left) {
		  ...
		  TRACET("insert remapped left mapping {} {}",
			 c.trans,
			 ext->get_laddr(),
			 val);
		  return btree.insert(
		...
		return btree.insert(c, std::move(it), laddr, val, nullptr
		  ...
		  state.remapped_extents.pop_front();
		      ...
		      ext->get_last_committed_crc()
                  ...
                  TRACET("insert remapped right mapping {} {}",
			 c.trans,
			 ext->get_laddr(),
			 val);
		  return btree.insert(
