...
private:
  rewrite_gen_t adjust_generation(
    ...
    if (is_root_type(type)) {
      ...
      if (gen < cold_tier_generations &&
          policy == write_policy_t::WRITE_THROUGH) {
        gen = cold_tier_generations;
      }
    }

    if (is_tracked && gen >= cold_tier_generations &&
        policy != write_policy_t::WRITE_BACK) {
      gen = cold_tier_generations - 1;
    }
    ...
    void init(JournalTrimmerImplRef &&_trimmer,
      ...
      if (_cold_cleaner) {
        ...
        logical_bucket = create_logical_bucket(
          ...
          get_conf<Option::size_t>("seastore_logical_bucket_demote_size"));
      }
    }
