...
class LRUMemoryCache : public MemoryCache {
  ...
  void remove_impl(CachedExtent &extent, bool need_to_promote) {
    ...
    TRACE("extent {}~{} with {}, need to promote: {}, promote_contents: {}",
          extent.get_paddr(), extent.get_length(), get_cache_state(extent),
          need_to_promote, promote_contents);
	...
	if (need_to_promote &&
	  ...
	  TRACE("move extent {} to pending promote list", extent.get_paddr());
	  ...
	  TRACE("release extent {}", extent.get_paddr());
	...
	TRACE("remove extent {} from pending promote list", extent.get_paddr());
	promote_contents -= extent.get_length();
    ...
    metrics.add_group(
      "cache",
      {
        ...
        sm::make_counter(
          ...
          sm::description("total extents pending promoted to the faster device")
        ),
        sm::make_counter(
          "pending_promotion_bytes",
          [this] { return promote_contents; },
          sm::description("total bytes pending promoted to the faster device")
        )
      }
    );
  }
