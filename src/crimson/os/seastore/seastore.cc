  ...
  ceph::os::Transaction::iterator &i)
{
  ...
  return fut.si_then([&, op](auto get_onode) {
    ...
    if ((op->op == Transaction::OP_CLONE
	  ...
	  || (op->op == Transaction::OP_TOUCH && is_temp_obj))
	...
	*ctx.transaction, i.get_oid(op->dest_oid)
	...
	auto p = removed_info.emplace(i.get_oid(op->oid), removed_info_t{laddr, id});
  ...
  extent_len_t dstoff)
{
  ...
  if (s_lc_id != d_lc_id) {
    auto hint_lc_id = std::max(s_lc_id, d_lc_id) - 1;
  ...
  ceph::os::Transaction::Op *op) const
{
  ...
  if (input_id == LOCAL_CLONE_ID_NULL) {
	...
	*ctx.transaction, ghobj
    ...
    return seastar::do_with(
      *p_ghobj,
      [this, &ctx, &onode, target_is_head, FNAME, input_id](auto &src_ghobj) {
	src_ghobj.hobj.snap = CEPH_NOSNAP;
	return onode_manager->get_latest_snap_and_head(
	  *ctx.transaction, src_ghobj
	).si_then([&onode, target_is_head, &ctx, FNAME, input_id](auto res) {
	  ...
	  if (head && get_onode_base(head).has_valid_prefix()) {
	    ...
	    DEBUGT("latest: {:d}", *ctx.transaction, get_onode_base(head));
	  }
	  if (hint == L_ADDR_NULL) {
	    hint = onode->get_data_hint();
