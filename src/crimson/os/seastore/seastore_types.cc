...
laddr_t laddr_t::get_next_hint(laddr_t laddr)
{
  ...
  if (// data: every time we want next data hint, make random + 1
      ...
      // metadata: when the offset is going to overflow to local snap id
      // we carry this bit to random field.
      ((l.low & LOW_OFFSET_MASK) == (U64MAX & LOW_OFFSET_MASK))) {
    // random is full, we don't expect it overflow to crush field
    // and reset random to 0.
    if (((l.high & HIGH_RANDOM_MASK) == HIGH_RANDOM_MASK) &&
	((l.low & LOW_RANDOM_MASK) == LOW_RANDOM_MASK)) {
      inc = false;
      l.high &= ~HIGH_RANDOM_MASK;
      l.low &= ~LOW_RANDOM_MASK;
    } else {
      // set all bits to 1 except for random field
      l.low |= ~LOW_RANDOM_MASK;
    }
  }
