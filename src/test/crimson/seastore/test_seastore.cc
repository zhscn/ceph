...
struct seastore_test_t :
  ...
  struct object_state_t {
    ...
    void touch(
      CTransaction &t,
      std::optional<ghobject_t> target_oid,
      std::optional<local_clone_id_t> clone_id) {
      t.touch(cid, oid, target_oid, clone_id);
    }
    ...
    void remove(
      SeaStoreShard &sharded_seastore,
      bool remove_coll = true) {
      CTransaction t;
      remove(t, remove_coll);
      sharded_seastore.do_transaction(
        coll,
        std::move(t)).get0();
    }
...
TEST_P(seastore_test_t, clone_range)
{
  run_async([this] {
    ...
    test_obj1.read(*sharded_seastore, 0, 4096 * 1024);
    auto &test_obj2 = get_object(make_oid(0, 10));
    test_obj2.touch(*sharded_seastore, std::nullopt, 2);
...
*/
    CTransaction t;
    auto buffer = bufferptr(buffer::create(16384));
    ::memset(buffer.c_str(), 'a', 16384);
    bufferlist bl;
    bl.append(buffer);
    test_obj2.write(*sharded_seastore, t, 0, std::move(bl));
    test_obj2.clone_range(*sharded_seastore, t, test_obj1, 8192, 16384, 8192);
    do_transaction(std::move(t));
...
TEST_P(seastore_test_t, recover_clone_range)
{
  run_async([this] {
    auto &test_obj1 = get_object(make_oid(0));
    test_obj1.write(*sharded_seastore, 0, 4096 * 1024, 'c');
    test_obj1.read(*sharded_seastore, 0, 4096 * 1024);
    auto &test_obj2 = get_object(
      make_oid(1, CEPH_NOSNAP, std::string(TEMP_RECOVERING_OBJ_PREFIX)));
    test_obj2.touch(*sharded_seastore, test_obj1.oid, 0);
    test_obj2.write(*sharded_seastore, 65536, 65536, 'b');
    test_obj2.write(*sharded_seastore, 1 << 18, 1 << 16, 'd');
    test_obj2.clone_range(*sharded_seastore, test_obj1, 98304, 4096, 98304);
    std::cout << "seastore_test_t.clone_range 1 clone_range" << std::endl;
...
/*    CTransaction t;
    ...
    test_obj1.remove(*sharded_seastore, false);
    test_obj2.rename(*sharded_seastore, test_obj1);
  });
}
