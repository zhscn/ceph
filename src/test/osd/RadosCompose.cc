...
struct Client {
  ...
  void init(const std::string &user, const std::string &conf_path) {
    int ret = 0;

    ret = cluster.init2(user.c_str(), "ceph", 0);
    if (ret < 0) {
      throw EX("init2 {}", ret);
    }

    ret = cluster.conf_read_file(conf_path.c_str());
    if (ret < 0) {
...
int main(int argc, const char **argv) {
  bpo::options_description desc("rados compose tests");
  desc.add_options()
      ("help", "desplay help message")
      ("cycle", bpo::value<int>()->default_value(100), "test cycle")
      ("conf", bpo::value<std::string>()->default_value("/etc/ceph/ceph.conf"),
       "patch to ceph.conf")
      ("user", bpo::value<std::string>()->default_value("client.admin"),
       "init user");
  ...
  auto cycle = vm["cycle"].as<int>();
  auto user = vm["user"].as<std::string>();
  auto config_path = vm["conf"].as<std::string>();

  try {
    Client client;
    client.init(user, config_path);
    client.run(cycle);
